...
## 架构概述
Hermes Desktop 是一个基于 Tauri v2 + Vue 3 的跨平台桌面 AI Agent 应用，
完整的 Hermes Agent 在底层运行，提供所有 AI 能力。
...
层次结构：
  ...
  应用逻辑层 (Rust 后端)
      ↕ 子进程启动
  AI Agent 层 (Python - Hermes Agent 核心) ← 最底层
      ↕ HTTPS
...
## 下一步行动
1. 安装依赖: cd /root/hermes-desktop && npm install
2. 开发运行: npm run tauri dev
3. 构建发布: npm run tauri build

## 关键技术决策
1. Hermes Agent 在底层运行，桌面应用只是 GUI 外壳
